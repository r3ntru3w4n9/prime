...
void QuadForest::construct_forest() {
    ...
    // std::cout << "Rows: " << maxRows << " Columns: " << maxCols << " Layers:
    // " << maxLayers << std::endl;
    ...
    for (size_t i = 0; i < num_nets; ++i) {
        ...
        for (size_t j = 0; j < segments.size(); j += 6) {
            qt.add_segment(segments[j], segments[j + 1], segments[j + 2],
                           segments[j + 3], segments[j + 4], segments[j + 5]);
        }
        qt.construct_tree();
...
void QuadForest::return_segments(Chip& chip) {
    for (size_t i = 0; i < qtrees.size(); ++i) {
        ...
        for (size_t j = 0; j < segments.size(); ++j) {
            net.addSegment(segments[j].get_xs(), segments[j].get_ys(),
                           segments[j].get_layer(), segments[j].get_xe(),
                           segments[j].get_ye(), segments[j].get_layer_end());
        }
    }
}
