...
unsigned NetSegment::get_length() const { return ABS(x_end - x_start) + ABS(y_end - y_start); }
...
void NetSegment::merge_segment(NetSegment& ns) { // merge two parallel segments
    ...
    if(get_direction()){ // vertical
        ...
        if (y_start > ns.get_ys()) y_start = ns.get_ys();
        if (y_end   < ns.get_ye()) y_end   = ns.get_ye();
    }
}
