...
Die::Die(unsigned x,unsigned y,unsigned z,cost_type t):_x(x),_y(y),_z(z),_area(_x*_y),_vol(_x*_y*_z),_CostType(t)
{
    _GridList.reserve(_vol);
    for(int i = 0;i < _vol;i++) { Grid* grid = 0; _GridList.push_back(grid); }
}

Die::~Die()
{
    for(int i = 0;i < _vol;i++) delete _GridList[i];
}

bool Die::add_net(unsigned idx1,unsigned idx2)
{
    if(_GridList[idx1] !=0 || _GridList[idx2] !=0) return false;//idx initialized
...
void Die::add_normal()
{
    for(int i = 0;i < _vol;i++) { if(_GridList[i]==0) _GridList[i] = new NormalGrid(i); }
}


int Die::A_star(const unsigned origin,IdxList& ans)//return the cost of the rout
{
    ...
    while (!propagate(x,target))
    {
        ...
        {
            std::cout << "Network " << get_x(origin) << " " << get_y(origin) << " " << get_z(origin) << " "
            << get_x(target) << " " << get_y(target) << " " << get_z(target) << " has no solution." << std::endl;
            clear_space(origin);
            clear_space(target);
            reset();
            return 0;
    ...
    for(int i = 0;i < ans.size();i++) clear_space(ans[i]);
    reset();
    return ret;
}
...
{
    unsigned x = get_x(pi); unsigned y = get_y(pi); unsigned z = get_z(pi);
    if(x>0)//left
...
{
    if(_GridList[x] == 0) _GridList[x] = new NormalGrid(x,target);
    else
    {
        delete _GridList[x];
...
bool Die::propagate(const unsigned pi,const unsigned target)//true means find target
{
    unsigned x = get_x(pi); unsigned y = get_y(pi); unsigned z = get_z(pi);
    int cost = _GridList[pi]->get_cost();
    if(x>0)//left
    {
        if(sub_propagate(cost,pi,pi-1,target)) return true;
    }
    if(x<(_x-1))//right
    {
        if(sub_propagate(cost,pi,pi+1,target)) return true;
    ...
    if(y>0)//back
    {
        if(sub_propagate(cost,pi,pi-_x,target)) return true;
    }
    if(y<_y-1)//front
    {
        if(sub_propagate(cost,pi,pi+_x,target)) return true;
    ...
    if(z>0)//down
    {
        if(sub_propagate(cost,pi,pi-_area,target)) return true;
    }
    if(z<_z-1)//up
    {
        if(sub_propagate(cost,pi,pi+_area,target)) return true;
...
inline bool Die::sub_propagate(const int x,const unsigned pi,const unsigned front,const unsigned target)//true means find target and x is pi's cost
{
    if(front == target)
    {
        _GridList[front]->assign_estimated(pi,x+1);//actually it should be assign_cost but for the convenience of virtual function
        return true;
    }
...
void Die::reset()
{
    ...
    for (int i = 0;i <_vol;i++) _GridList[i]->reset();
}
