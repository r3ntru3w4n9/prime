...
{
public:
    Router(std::fstream& inputfile,std::fstream& outputfile):_outputfile(outputfile),_err(0),_cost(0)
    {
        // read inputfile
        int x,y,z;
        inputfile >> x >> y >> z;
        ...
        while(inputfile >> x1 >> y1 >> z1 >> x2 >> y2 >> z2)
        {
            ...
            {
                std::cerr << "invalid coordinate: " << x1 << " " << y1 << " " << z1 << " " << x2 << " " << y2 << " " << z2 << std::endl;
                _err = true;
            }
        }
        _die->add_normal();
    ...
    void rout()
    {
        for(int i=0;i<_NetList.size();i++)
        {
            ...
            {
                _outputfile << _die->get_x(ans[j]) << " " << _die->get_y(ans[j]) << " " << _die->get_z(ans[j]) << std::endl;
            }
